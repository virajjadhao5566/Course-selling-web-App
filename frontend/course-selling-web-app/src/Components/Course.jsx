...
const Course = () => {
    ...
    let course = courses.find(c => c.id === courseId.courseId - '0')
    if (!course) {
        return <div>
            ...Loading
        </div>
    ...
    return (
        <div>
            <div style={{
                ...
                <UpdateCourse setCourses={setCourses} courses = {courses} course = {course}/>
            </div>
        </div>
    )
}
                            ...
                            fetch('http://localhost:3000/admin/courses/' + props.course.id, {
                                ...
                                response.json().then((data) => {
                                    ...
                                    for(let i = 0;i<courses.length;i++){
                                        if(courses[i].id === props.course.id){
                                            updatedCourses.push({
                                                id:props.course.id,
                                                title,
                                                description,
